...
const RevenueByLocation = () => {
  ...
  return (
    <div className={`rounded-xl shadow-sm border p-4 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
      <h3 className={`text-base font-semibold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Revenue by Location</h3>
      
      {/* World Map */}
      <div className="relative mb-4">
        <svg viewBox="0 0 200 100" className={`w-full h-16 ${isDarkMode ? 'text-gray-600' : 'text-gray-300'}`}>
        ...
        {locations.map((location, index) => (
          <div key={index} className="py-1">
            <div className="flex items-center justify-between mb-1">
              <span className={`text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{location.city}</span>
              <span className={`text-xs font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{location.revenue}</span>
            </div>
            <div className={`w-full h-0.5 rounded-full ${isDarkMode ? 'bg-gray-600' : 'bg-gray-200'}`}>
              <div 
                className={`h-0.5 rounded-full transition-all duration-1000 ease-out ${
