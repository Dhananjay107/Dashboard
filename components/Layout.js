...
import UltraResponsiveLayout from './UltraResponsiveLayout';
...
const Layout = ({ children }) => {
  ...
  // Use ultra-responsive layout for mobile devices
  if (isMobile) {
    return <UltraResponsiveLayout>{children}</UltraResponsiveLayout>;
  }

  return (
    <div className={`flex h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      {/* Sidebar */}
      <div className={`${isMobile ? (sidebarOpen ? 'w-full' : 'w-16') : (sidebarOpen ? 'w-56' : 'w-16')} transition-all duration-300 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg flex flex-col ${isMobile ? 'h-auto' : 'h-full'}`}>
        <div className="p-6">
        ...
        <nav className="px-4 space-y-8 flex-1 overflow-y-auto scrollbar-hide pb-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              {/* Default - Active */}
              <Link href="/orders">
                <div className={`flex items-center px-2 py-2 rounded cursor-pointer relative ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'} hover:${isDarkMode ? 'bg-gray-600' : 'bg-gray-200'} transition-colors duration-200`}>
              ...
              {/* eCommerce */}
              <Link href="/">
                <div className={`flex items-center px-2 py-2 rounded cursor-pointer ${isDarkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-50'} transition-colors duration-200`}>
      ...
      <div className={`${isMobile ? 'w-full' : 'flex-1'} flex ${isMobile ? 'flex-col' : ''}`}>
        {/* Main Content */}
        <div className={`${isMobile ? 'w-full' : 'flex-1'} flex flex-col`}>
          {/* Header */}
          <header className={`shadow-sm border-b px-4 sm:px-6 py-4 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className="flex items-center justify-between">
              ...
              <div className={`flex items-center ${isMobile ? 'space-x-2' : 'space-x-4'}`}>
                ...
                <div className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-600'} ${isMobile ? 'hidden sm:block' : ''}`}>
              ...
              <div className={`flex-1 flex justify-center ${isMobile ? 'max-w-xs mx-2' : 'max-w-md mx-8'}`}>
                <div className="relative w-full">
                  ...
                  <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                    <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-400'} ${isMobile ? 'hidden' : ''}`}>⌘/</span>
                  </div>
                </div>
              </div>
              ...
              <div className={`flex items-center ${isMobile ? 'space-x-1' : 'space-x-1'}`}>
                ...
                {/* Clock Icon */}
                <button className={`p-2 rounded-lg ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'} ${isMobile ? 'hidden sm:block' : ''}`}>
          ...
          {/* Main Content Area */}
          <main className={`${isMobile ? 'flex-1' : 'flex-1'} overflow-auto scrollbar-hide`}>
            <div className={`${isMobile ? 'min-h-screen pb-4' : 'min-h-full'} ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
              {children}
            </div>
          </main>
        </div>
        ...
        {rightSidebarOpen && !isMobile && (
          <div className="w-56 transition-all duration-300 flex-shrink-0">
            <RightSidebar />
          </div>
        )}
      </div>
