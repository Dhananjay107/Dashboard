  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {salesData.map((item, index) => (
          <div key={index} className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
            ...
            <div className="text-right">
              ...
              <div className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{item.percentage}%</div>
            </div>
          </div>
        ))}
      </div>
...
export default MobileTotalSalesChart;
